...
class R6s(Pins):
    ...
    def __init__(self):
        df = pd.read_csv(join(self.data_path, 'Pine Point Full_master.csv'))
        ...
        Y = (Y - np.min(Y))/S
        X = 1 - (X - np.min(X))/S
        # Add an offset of 6% on the sides and 3% on the top and bottom.
        X = (X + .03)/1.03
        Y = (Y + .03)/1.03
        pins = []
        ...
        super(R6s, self).__init__(pins, [100,100], 1)
